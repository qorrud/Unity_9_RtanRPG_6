...
namespace Revolver_6
{
    internal static class Battle
    {
        ...
        //플레이어의 턴일때 실행되는 함수입니다. 플레이어의 턴이 끝나면  turn 은 false로 변경해서 적의 턴으로 넘어갑니다.
        {
            ...
            int error = (int)Math.Ceiling(Player_baseAttack * 0.1); //공격력의 오차 설정
            int min = Player_baseAttack - error; //최소 데미지
            int max = Player_baseAttack + error; //최대 데미지
            ...
            else //죽지 않았다면 남은 체력을 표시
            {
                ...
                Console.Clear();
                if (Player_turn == false) //아 근데 저거 bool값 굳이 필요한가 싶기도 하고..
                {
                    Monster_Pahse(); //몬스터 페이즈 실행
        ...
        internal static void Monster_Pahse() //몬스터 턴입니다.
        {
            Typing("Red", "Battle!\n");
            foreach (MonsterData.MonsterStat monster in MonsterData.MonsterList) // 몬스터 정보입니다.
            {
                if (monster.HP == 0)
                {
                    Typing("white", "Lv.", "red", $"{monster.Level}", "white", "은 사망했기 때문에 공격 할 수 없다!");
                    ...
                    Typing("white", $"{Player.name}을(를) 맞췃습니다.");
